(function () {
  ...
  function startUnitBrowserRealtimeStream() {
    ...
    techUnitEventSource.addEventListener('unit-browser-change', (event) => {
      const detailContainer = document.querySelector('[data-tech-unit-detail-refresh-url]');

      if (detailContainer) {
        let eventUnitId = null;

        try {
          eventUnitId = Number(JSON.parse(event.data || '{}').unitId);
        } catch (_error) {
          // Malformed events fall through to the normal refresh path.
        }

        const currentUnitId = Number(
          detailContainer.querySelector('[data-unit-record]')?.getAttribute('data-unit-id')
        );

        if (Number.isSafeInteger(currentUnitId) && currentUnitId > 0
          && Number.isSafeInteger(eventUnitId) && eventUnitId > 0
          && currentUnitId !== eventUnitId) {
          return;
        }
      }
